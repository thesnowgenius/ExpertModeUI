...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.getElementById('expertForm').addEventListener('submit', async (e)=>{
    ...
    if (multi) {
      payload = {
        riders: ridersRaw.map(r => ({name: r.name || 'Rider', age: r.age, category: r.category && r.category !== 'None' ? r.category : null})),
        resorts: resortsRaw.map(r => ({ name: r.resort_name || r.resort_id, days: r.days, no_blackouts: !r.blackout_ok })),
        no_weekends: !!document.getElementById('noWeekends')?.checked,
        no_blackouts: !!document.getElementById('noBlackouts')?.checked,
        mode: 'auto'
      };
    } else {

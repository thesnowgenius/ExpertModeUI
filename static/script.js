/* Snow Genius — Expert Mode (Multi Only, Minimal)
   - Posts to POST {API_BASE}{API_ENDPOINT} = /score_multi_pass
   - Payload: { riders:[{age,category}], resort_days:[{resort,days,blackout_ok}] }
*/

const API_BASE = (window.API_BASE || "").replace(/\/$/, "");
const API_ENDPOINT = window.API_ENDPOINT || "/score_multi_pass";
...
function removeRider(id) { state.riders = state.riders.filter(x => x.id !== id); renderRiders(); }
function updateRider(id, patch) { const n = state.riders.find(x => x.id === id); if (n) Object.assign(n, patch); }
...
function renderRiders() { ridersEl.innerHTML = ""; if (state.riders.length === 0) addRider(); state.riders.forEach(r => ridersEl.appendChild(riderRow(r))); }

/* ----- Resorts ----- */
function addResort(r = { id: uid(), resort_id: "", label: "", days: 1, blackoutOk: false }) { state.resorts.push(r); renderResorts(); }
function removeResort(id) { state.resorts = state.resorts.filter(x => x.id !== id); renderResorts(); }
function updateResort(id, patch) { const n = state.resorts.find(x => x.id === id); if (n) Object.assign(n, patch); }
...
function renderResorts() { resortsEl.innerHTML = ""; if (state.resorts.length === 0) addResort(); state.resorts.forEach(r => resortsEl.appendChild(resortRow(r))); }

/* ----- Results ----- */
function renderResults(data) {
  resultsEl.innerHTML = "";
  if (data?.best_combo || data?.total_cost != null) {
    const combo = Array.isArray(data.best_combo) ? data.best_combo.join(" + ") : (data.best_combo ?? "—");
...
async function handleSubmit(e) {
  ...
  // IMPORTANT: send "resort" (normalizer also accepts resort_id)
  const resort_days = state.resorts
    .filter(r => r.resort_id && r.days > 0)
    .map(r => ({ resort: r.resort_id, days: Number(r.days), blackout_ok: !!r.blackoutOk }));

  const payload = { riders, resort_days }; // no "mode" needed for multi-only
